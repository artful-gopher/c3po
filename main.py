    ...
    with open('/tmp/tosheet.csv') as csvfile:
        ...
        for row in readCSV:
            if(row[0] == mondays_date.strftime("%Y-%m-%d")):
                ...
                response = client.chat_postMessage(channel=os.environ['CHANNEL_ID'], text="Hi "+os.environ['SLACK_HANDLE']+ " Quick Reminder: This week\'s EC to WC TO pair is "+name1+" and "+name2+" :robot_face:")
